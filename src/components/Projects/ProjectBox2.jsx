...
function ProjectBox2({ flowerClass, bg, title, description, src }) {
  const { theme } = useTheme();

  return (
    <div
      className={`${theme === "dark" ? "text-white border-white" : "text-black border-black"} border-2 flex relative rounded-xl px-4 py-2 pb-8 sm:px-6 sm:py-4 sm:pb-12 hover:scale-101 transition-all duration-200 hover:z-9999999 `}
    >
      <div className="mr-4 sm:mr-20">
        ...
        <p className="text-xs sm:text-sm w-2xs sm:w-md">{description}</p>
      </div>
